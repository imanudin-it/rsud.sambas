...
<div class="section-body">
    ...
    <p class="section-lead">Manage {{ $title }}</p>
           ...
           <div class="bg-whitesmoke p-2 text-right mb-3">
                <button class="btn btn-primary" type="button" data-toggle="collapse" data-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
                  ...
                  <div class="collapse" id="collapseExample" align="left">
                    <hr class="text-muted">
                    <div class="card p-0">
                        <form action="/dashboard/services/details" method="POST" enctype="multipart/form-data">
                            @csrf
                        <div class="row">
                            ...
                            <div class="col-md-8 col-lg-8">
                                ...
                                <div class="form-group mb-4">
                                    <label> Name </label>
                                    <input type="text" class="@error('name') is-invalid @enderror form-control " name="name" id="title"  value="{{ old('name') }}" autocomplete="off" required>
                                    <input type="hidden" class="form-control " name="service_id" value="{{ $service->id }}" autocomplete="off" required>
                                </div>
                                <div class="form-group mt-0">
                                    <label> Description </label>
                                    <input type="hidden" id="descriptions" class="@error('descriptions') is-invalid @enderror" name="descriptions"  value="{{ old('description') }}" required>
                ...
                <table class="table table-striped dataTable no-footer" id="table-2" role="grid" aria-describedby="table-1_info">
                   ...
                   <tr>
                    ...
                    <td> {{ $row->name }} <br>
                    <small><p class="text-truncate">{!! $row->descriptions !!}</p></small></td>
                     <td> <div class="btn-group">
                        <a href="./details/{{ $row->id }}/edit" class="btn btn-warning" title="Edit"><i class="fa fa-edit"></i></a>
                        <button type="button" onclick="confirmDeleteDetails({{ $row->id }})" class="btn btn-danger" title="Hapus ?"><i class="fa fa-times"></i></button>
                        </div> </td>
                   </tr>
                        @endforeach
                    </tbody>
